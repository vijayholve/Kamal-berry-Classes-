...
>
  ...
  <MenuItem disabled sx={{ fontWeight: "bold", color: "#ffcc00" }}>
    Advanced Courses
  </MenuItem>
  {advancedCourses.map((category, index) => {
  ...
  <MenuItem disabled sx={{ fontWeight: "bold", color: "#ffcc00", marginTop: "10px" }}>
    Beginner Courses
  </MenuItem>
  {beginnerCourses.map((category, index) => {
    const categoryPath = `/courses/${category.replace(/\s+/g, "-").toLowerCase()}`;
    return (
      <Link key={index} to={categoryPath} style={{ textDecoration: "none", color: "white" }}>
        <MenuItem
